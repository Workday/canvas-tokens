...
export const changeValuesToCSSVars = (
  ...
  const isFallback = typeof token.original.oldValues === 'object';
  const fallbackValues = isFallback
    ? (Object.values(oldValues).filter(item => typeof item === 'string') as string[])
    : [];
    ...
    return Object.entries<string>(originalValue).reduce(
      (acc: Record<string, string>, [key, value]: string[]) => {
        ...
        const newValue = isFallback
          ? `var(${cssVarName}, ${generateFallbacks(fallbackValues, oldValues.raw || value)})`
          : cssVarName;
  ...
  const rawValue = !(typeof originalValue === 'string' && originalValue.startsWith('{'))
    ...
    : token.value;
  const newValue = isFallback
    ? `var(${cssVarName}, ${generateFallbacks(fallbackValues, oldValues.raw || rawValue)})`
    : cssVarName;
  // Use lowercase to match CSS variable names generated by name/cti/kebab transform
  return newValue;
};
