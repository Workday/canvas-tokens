...
describe('transforms', () => {
  ...
  it('should flatten resolved nested oklch with slash alpha to relative syntax', () => {
    const result = transforms['value/flatten-oklch'].transformer(
      {
        ...defaultToken,
        value: 'oklch(oklch(0 0 0 / 1) / 0.03)',
  ...
  it('should flatten resolved nested oklch with comma alpha to relative syntax', () => {
    const result = transforms['value/flatten-oklch'].transformer(
      {
        ...defaultToken,
        value: 'oklch(oklch(63.3% 0.21 230 / 1), 0.25)',
        original: {value: 'oklch({base.palette.blue.500} / 0.25)'},
      },
      defaultOptions
    );

    expect(result).toMatch(/^oklch\(.+ \/ 0\.25\)$/);
  });

  it('should flatten unresolved oklch token references to relative syntax', () => {
    const result = transforms['value/flatten-oklch'].transformer(
      {
        ...defaultToken,
    ...
    expect(result).toBe('oklch(from {sys.color.shadow.base.light} l c h / 0.03)');
  });

  it('should output transparent for zero alpha nested oklch', () => {
    const result = transforms['value/flatten-oklch'].transformer(
      {
        ...defaultToken,
        value: 'oklch(oklch(63.3% 0.21 230 / 1), 0)',
        original: {value: 'oklch({base.opacity.0} / 0)'},
      },
      defaultOptions
    );
  ...
  it('should flatten nested oklch inside box-shadow strings', () => {
        ...
        value: [
          {
            ...
            blur: '0.25rem',
            spread: '-0.0625rem',
            color: 'oklch(oklch(0 0 0 / 1) / 0.03)',
            type: 'dropShadow',
          },
          {
            x: '0',
            ...
            blur: '0.5rem',
            spread: '0',
            color: 'oklch(oklch(0 0 0 / 1) / 0.02)',
            type: 'dropShadow',
          },
        ],
        path: ['sys', 'depth', '1'],
    ...
    expect(result).toBe(
      '0 0.0625rem 0.25rem -0.0625rem oklch(0 0 0 / 0.03), 0 0.125rem 0.5rem 0 oklch(0 0 0 / 0.02)'
    );
  });
