...
- [Direct Transformation Formatters](#direct-transformation-formatters)
  ...
  - [Tokens Info Formatters](#tokens-info-formatters)
...
## Tokens Info Formatters

#### json/tokens-info-export

Located in `formatTokensInfoExport`. Outputs an array of objects with detailed information about
each token as prettified JSON.
...
#### csv/tokens-info-export

Located in `formatTokensInfoExportCSV`. Outputs a CSV file with one row per token and columns for
key metadata.
