...
<Meta title="Docs/Brand/Brand Tokens (New)" />
...
const styles = {
  backgroundColor: `var(${brand.primary600})`,
  color: `var(${system.color.fg.inverse})`,
};
```
...
.button {
  background-color: var(--cnvs-brand-primary-600);
